...
export default async function exportRoutes(fastify) {
  fastify.post('/api/export/:name', async (request, reply) => {
    ...
    try {
      // Use --columns=9999 to prevent pandoc from wrapping/sizing columns based on content length
      await execAsync(
        `pandoc "${inputPath}" -f markdown-auto_identifiers --columns=9999 --lua-filter="${LUA_FILTER}" --lua-filter="${TABLE_FILTER}" --reference-doc="${REFERENCE_DOC}" -o "${outputPath}"`,
      );
